...
class LenderReviewPage extends StatefulWidget {
  const LenderReviewPage({super.key});

  @override
  State<LenderReviewPage> createState() => _LenderReviewPageState();
}
...
class _LenderReviewPageState extends State<LenderReviewPage> {
  ...
  Future<void> _refresh({bool silent = false}) async {
    try {
      final raw = await api.getRequests(role: 'owner');
      if (!mounted) return;
      final list = raw
                  ...
                  return RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final borrowerId = (req['borrower_id'] as num?)?.toInt();
                        final borrower = req['borrower_name']?.toString() ?? 'Borrower';
                            ...
                            children: [
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(borrower, style: const TextStyle(fontWeight: FontWeight.bold)),
                                        Text('borrowed $itemTitle', style: TextStyle(fontSize: 13, color: Colors.grey[600])),
                              ...
                              SizedBox(
                                ...
                                child: ElevatedButton(
                                  onPressed: requestId == null || borrowerId == null
                                      ? null
                                      : () {
                                          Navigator.push(
                                            context,
                                  ...
                                  child: const Text('Review Borrower'),
                                ),
                              ),
                            ],
                          ),
